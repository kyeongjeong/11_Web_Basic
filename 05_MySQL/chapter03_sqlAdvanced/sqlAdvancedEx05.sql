...
SELECT *
FROM CAR
WHERE CAR_CD IN (SELECT CAR_CD    # CAR CD = 이 아니라 IN 써야 함(서브쿼리 내에 SELECT가 여러 개)
								FROM RENT
								WHERE RENT_PERIOD >= 5 );
...
#(참고)
# 예시 6) 자동차테이블의 백업 테이블을 생성하고 데이터를 마이그레이션하기.
...
#INSERT INTO CAR_BAK VALUES(SELECT * FROM CAR); #안돌아감
...
WHERE CAR_CD NOT IN (SELECT CAR_CD FROM RENT);
...
# 예시 9) 가장 비싼 차량3개의 가격을 10% 할인하기 (테이블명의 중첩될 경우 AS로 테이블명 지정해야 사용 가능)
UPDATE CAR
SET PRICE = PRICE * 0.9
WHERE CAR_CD IN (SELECT TEMP.CAR_CD
								FROM (SELECT CAR_CD 
											FROM CAR
                                            ORDER BY PRICE DESC
                                            LIMIT 3) AS TEMP);
...
#SELECT PRICE - AVG(PRICE) #불가능함
SELECT PRICE - (SELECT AVG(PRICE) FROM CAR)
FROM CAR
WHERE CAR_NM = 'k5';
...
FROM (SELECT * 
            ...
            WHERE CAR_CD IN (SELECT CAR_CD
											FROM RENT
                                            WHERE RENT_PERIOD >= 5)) AS T2;
					
# 예시 3) 현대 자동차의 평균 렌트일 조회하기.
SELECT CAR_CD
FROM CAR
WHERE BRAND_NM = '현대';
...
SELECT ROUND(AVG(T3.RENT_PERIOD))
FROM  (SELECT *
			FROM RENT
			WHERE CAR_CD IN (SELECT CAR_CD
											FROM CAR
											WHERE BRAND_NM = '현대')) AS T3;