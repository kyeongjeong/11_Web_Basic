...
SELECT
	P.PRODUCT_NM,
    SUM(O.ORDER_GOODS_QTY) AS TOTAL_ORDER
FROM
	ORDER_TB O
    INNER JOIN PRODUCT_TB P
	ON O.PRODUCT_CD = P.PRODUCT_CD
...
FROM
	...
	ON O.PRODUCT_CD = P.PRODUCT_CD
WHERE
	P.PRODUCT_NM='APPLE 2020 맥북 에어 13';

# 11) 1000000 ~ 2000000가격의 판매된 상품별로 상품의 코드 , 이름 , 총 판매량 조회하기
SELECT
	P.PRODUCT_CD,
    P.PRODUCT_NM,
    SUM(O.ORDER_GOODS_QTY) AS TOTAL
FROM
	ORDER_TB O
    INNER JOIN MEMBER_TB M
    ON O.MEMBER_ID = M.MEMBER_ID
	...
	ON O.PRODUCT_CD = P.PRODUCT_CD
WHERE
	P.PRICE BETWEEN 1000000 AND 2000000
GROUP BY
	P.PRODUCT_CD,
    P.PRODUCT_NM;
