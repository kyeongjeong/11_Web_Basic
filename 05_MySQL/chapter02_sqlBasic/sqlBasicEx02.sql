...
SELECT * FROM PRODUCT WHERE PRODUCT_CD = 'P10001';
		
# 상품코드가 'P10001'이 아닌 상품 전체 컬럼 조회
SELECT * FROM PRODUCT WHERE PRODUCT_CD <> 'P10001';
			
# 상품등록 날짜가 2021년 이후부터의 상품의 전체 컬럼 조회		
SELECT * FROM PRODUCT WHERE REG_DT >= '2021-01-01';
...
*/
SELECT REG_DT FROM PRODUCT;	
SELECT DISTINCT REG_DT FROM PRODUCT;		
...
SELECT * FROM PRODUCT WHERE PRODUCT_CD = 'P10001' OR PRODUCT_CD ='P10003' OR PRODUCT_CD ='P10005';
		
# 상품가격이 20000 ~ 50000인 상품의 전체컬럼 조회
SELECT * FROM PRODUCT WHERE PRICE >= 20000 AND PRICE <= 50000;
		
# 상품가격이 20000 ~ 50000이 아닌 상품의 전체컬럼 조회
SELECT * FROM PRODUCT WHERE NOT(PRICE >= 20000 AND PRICE <= 50000);
		
# 상품가격이 20000 ~ 50000이면서 2021년도 등록상품의 전체컬럼 조회		
SELECT * FROM PRODUCT WHERE (PRICE >= 20000 AND PRICE <= 50000) AND (REG_DT >= '2021-01-01' AND REG_DT <= '2021-12-31');
		
# 상품 등록날짜가 2020년도 1~3월 , 2021년도 1~3월 등록상품의 전체컬럼 조회
SELECT * FROM PRODUCT WHERE (REG_DT >= '2020-01-01' AND REG_DT <= '2020-03-31') OR (REG_DT >= '2021-01-01' AND REG_DT <= '2021-03-31');
...
# 상품가격이 20000 ~ 50000이 아닌 상품의 전체컬럼 조회
#NOT PRICE BETWEEN이나 PRICE NOT BETWEEN 둘 다 가능. 순서는 상관없음.
SELECT * FROM PRODUCT WHERE NOT PRICE BETWEEN 20000 AND 50000;
		
# 2021년에 등록된 상품의 전체컬럼 조회		
SELECT * FROM PRODUCT WHERE REG_DT BETWEEN '2021-01-01' AND '2021-12-31';
		
# 2021년에 등록되지 않은 상품의 전체컬럼 조회		
SELECT * FROM PRODUCT WHERE NOT REG_DT BETWEEN '2021-01-01' AND '2021-12-31';
		
# 상품코드가 'P10001' , 'P10003' , 'P10005'인 상품의 전체컬럼 조회
SELECT * FROM PRODUCT WHERE PRODUCT_CD IN ('P10001' , 'P10003' , 'P10005');
		
# 상품코드가 'P10001' , 'P10003' , 'P10005'이 아닌 상품의 전체컬럼 조회
SELECT * FROM PRODUCT WHERE	NOT PRODUCT_CD IN ('P10001' , 'P10003' , 'P10005');			
...
SELECT * FROM PRODUCT WHERE PRODUCT_NM LIKE '%USB%';
		
# 상품명이 'USB'로 시작하는 상품의 전체 컬럼 검색
SELECT * FROM PRODUCT WHERE PRODUCT_NM LIKE 'USB%';
		
# 상품명이 'GB'로 끝나는 상품의 전체 컬럼 검색
SELECT * FROM PRODUCT WHERE PRODUCT_NM LIKE '%GB'; 
...
# +@ 등록일이 같은 경우, 가격이 낮은 순으로 정렬
SELECT * FROM PRODUCT ORDER BY REG_DT DESC, PRICE;
		
# 2021년도에 등록된 상품의 전체 컬럼 검색 (가격이 높은 순서대로 정렬)
SELECT * FROM PRODUCT WHERE REG_DT BETWEEN '2021-01-01' AND '2021-12-31' ORDER BY PRICE DESC;
...
SELECT * FROM PRODUCT ORDER BY REG_DT DESC LIMIT 3; 
 
# 최신에 등록된 3개의 상품의 전체 컬럼 조회 (2번째 래코드 이후부터 조회)
SELECT * FROM PRODUCT ORDER BY REG_DT DESC LIMIT 1,3;