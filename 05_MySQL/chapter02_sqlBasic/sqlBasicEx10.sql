...
# ORDER_GOODS_QTY의 값들 중 가장 큰 값을 가져옴
SELECT MAX(ORDER_GOODS_QTY) FROM ORDER_TB;

# (MIN함수 예시)한번에 주문량이 제일 적었던 주문량을 조회하기.
SELECT MIN(ORDER_GOODS_QTY) FROM ORDER_TB;		
		
# (COUNT함수 + WHERE구문 예시) 배송이 완료된 주문건수를 조회하기
SELECT COUNT(*) FROM ORDER_TB WHERE DELIVERY_STATUS='배송완료';
		        		
# (SUM함수 + WHERE구문 예시) 'PRODUCT1'상품의 총 판매 수량을 조회하기
SELECT SUM(ORDER_GOODS_QTY) FROM ORDER_TB WHERE PRODUCT_CD='PRODUCT1';
		
# (AVG함수 + WHERE구문 예시)'PRODUCT1'상품의 평균 주문량을 조회하기
SELECT AVG(ORDER_GOODS_QTY) FROM ORDER_TB WHERE PRODUCT_CD='PRODUCT1';     
...
SELECT MEMBER_ID, COUNT(*) FROM ORDER_TB GROUP BY MEMBER_ID;     
		
# (GROUP BY 예시2) 배송상태별로 개수를 조회하기
SELECT COUNT(*) FROM ORDER_TB GROUP BY DELEVEY_STATUS; 
 
# (GROUP BY + WHERE구문 예시)배송완료가 아닌 배송상태별로 개수를 조회하기
SELECT COUNT(*) FROM ORDER_TB WHERE DELIVERY_STATUS<>'배송완료' GROUP BY DELEVEY_STATUS;  
        
# (GROUP BY + WHERE구문 + ORDER BY + LIMIT 예시) 'PRODUCT4' , 'PRODUCT5' , 'PRODUCT6'이 아닌 상품별로 주문상품의 총 주문수량을 조회한뒤 가장 많이 주문된 상품 3개만 조회하기
SELECT PRODUCT_CD, SUM(ORDER_GOODS_QTY) AS TOTAL_GOODS_QTY FROM ORDER_TB WHERE PRODUCT_CD NOT IN('PRODUCT4' , 'PRODUCT5' , 'PRODUCT6')  GROUP BY PRODUCT_CD ORDER BY SUM(ORDER_GOODS_QTY) DESC LIMIT 3;
  
# (GROUP BY + 글자함수 사용예시) 연도별로 총 주문건수 , 총 주문수량 , 평균 주문수량 조회하기
SELECT SUBSTRING(ORDER_DT, 1, 4) AS YEAR, COUNT(ORDER_GOODS_QTY), SUM(ORDER_GOODS_QTY), AVG(ORDER_GOODS_QTY) FROM ORDER_TB GROUP BY YEAR;

# (GROUP BY 2개이상 그룹화 예시) 사용자별로 주문상품별로 주문상품의 총수량을 조회하기
# 그룹핑을 먼저 하고, 그 안에서도 세분화하는 것임. ex) 경기도 남자, 경기도 여자, 서울 남자, 서울 여자
SELECT MEMBER_ID, PRODUCT_CD, SUM(ORDER_GOODS_QTY) FROM ORDER_TB GROUP BY MEMBER_ID, PRODUCT_CD ORDER BY MEMBER_ID, PRODUCT_CD;
...
#WHERE		
		#SUM(ORDER_GOODS_QTY) >= 10    #GROUP BY에 대한 컬럼의 조건은 HAVING으로 사용
