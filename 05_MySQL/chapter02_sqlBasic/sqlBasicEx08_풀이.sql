...
SELECT * FROM BOOK ORDER BY TITLE, PRICE DESC;
		
# 3) 책 전체 컬럼을 조회하기 (책 제목 ㄱ~ㅎ 순서 , 저자명 ㄱ~ㅎ순서)
SELECT * FROM BOOK ORDER BY TITLE, AUTHOR;
		
# 4) 책 전체 컬럼을 조회하기 (가장 많이 팔린 책 순서 , 책 제목 ㄱ~ㅎ 순서) 
SELECT * FROM BOOK ORDER BY SALES_CNT DESC, TITLE;
		
# 5) 책 전체 컬럼을 조회하기 (최신 등록된 순서)
SELECT * FROM BOOK ORDER BY PUBLISHING_DT DESC;
		
# 6) 책 전체 컬럼을 조회하기 (최신 등록된 순서 , 책 제목 ㄱ~ㅎ순서)
SELECT * FROM BOOK ORDER BY PUBLISHING_DT DESC, TITLE;
		
# 7) 제목,가격,할인률을 조회하기 컬럼을 조회하기 (할인률이 높은 순서 , 가격이 낮은 순서)
SELECT TITLE, PRICE, DISCOUNT_RATE FROM BOOK ORDER BY DISCOUNT_RATE DESC, PRICE;
		
# 8) 제목,가격,출판사,판매량을 조회하기(가장 많이 팔린 3곳)
SELECT TITLE, PRICE, PUBLISHING_COM, SALES_CNT FROM BOOK 	ORDER BY SALES_CNT DESC LIMIT 3;
		
# 9) 제목,가격,출판사,판매량을 조회하기(가장 적게 팔린 3곳)
SELECT TITLE, PRICE, PUBLISHING_COM, SALES_CNT FROM BOOK 	ORDER BY SALES_CNT LIMIT 3;

# 10) 판매량이 100개 이하인 책 전체 컬럼을 조회하기
SELECT * FROM BOOK WHERE SALES_CNT < 100;		

# 11) 가격이 10000~20000사이인 책의 전체컬럼을 조회하기
SELECT * FROM BOOK WHERE PRICE BETWEEN 10000 AND 20000;
		
# 12) 가격, 가격에 부가세를 10%를 계산한 컬럼과 , 부가세를 10%를 더한 컬럼조회하기
SELECT PRICE, PRICE+PRICE*10 AS TOTAL_PRICE FROM BOOK;
		
# 13) 가격 , 할인률 , 가격에 할인률을 적용한 컬럼을 조회하기
SELECT PRICE, DISCOUNT_RATE, PRICE-(PRICE*DISCOUNT_RATE)/100 AS TOTAL_PRICE FROM BOOK;
		
# 14) 가격 , 할인률 , 10%부가세 , 가격에 할인률을 적용하고 원가의 10%를 더한 책의 전체 컬럼을 조회하기
SELECT TITLE, PRICE, DISCOUNT_RATE,  PRICE-(PRICE*DISCOUNT_RATE)/100,  PRICE - (PRICE * DISCOUNT_RATE) / 100 + PRICE * 0.1 AS TOTAL_PRICE FROM BOOK;
